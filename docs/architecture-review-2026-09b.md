...
`§七缺口#3` `test_keys.py:116-117` 空壳测试（真测试在 `:120`，同名不同类，假阳性诱饵）—— 本轮已删除 ·
`§七缺口#4` 竞速心跳注入与客户端断开强制结算**曾无专属测试**—— 本轮已补齐，见第十三节 ·
...
## 十三、§六 第 12 项补齐 + WebUI 流畅度实测（2026-09-08）

### 13.1 两条此前无专属测试的路径

`tests/test_stream_lifecycle.py`（5 例）：

- **竞速窗口心跳注入**：`_stream_response` 把 `race_stream` 包进
  `asyncio.wait({race_task}, timeout=race_heartbeat)` 循环，静默期注入
  `: keep-alive`。此前只有 `test_ops.py` 覆盖 `_drain` 层（"胜出之后上游静默"），
  而首帧等待最长的那段（winner 诞生之前，可长达 `stream_first_byte_timeout` 180s）
  完全没测过。断言用"心跳必须全部先于首个 `data:` 帧"来证明它确实来自竞速窗口
  而不是 `_drain`。另测：客户端在心跳 yield 点断开时竞速 task 必须被取消（防 fd 泄漏）。
- **客户端断开强制结算**：`_stream_response` 的 finally 里 `not settled["done"]`
  分支。此前测试里 aclose / GeneratorExit / athrow 零命中。断言日志不得滞留
  pending、必须记一次失败、结算必须带 `reservation=1`；并配一条反向守卫
  "正常完成只结算一次"（settled 标记互斥）。

三条证伪跑全部产出**干净的断言失败**（不是超时），还原后逐字节一致。

### 13.2 证伪跑第一次暴露的守卫缺陷

第一次跑「拿掉心跳注入」时守卫**挂死到超时**而不是变红：`await gen.__anext__()`
在没有心跳的情况下永远等在 `asyncio.wait` 上。修法是给消费生成器加
`asyncio.wait_for` 并在超时分支显式 `aclose()`（被 wait_for 取消过 `__anext__`
的异步生成器处于"仍在运行"状态，留给测试收尾去关会卡住整个进程）。

**一个不会失败只会卡住的守卫等于没有守卫** —— 这条写进了测试文件的注释。

### 13.3 WebUI 流畅度：浏览器侧实测（补上此前只有源码形态守卫的弱验证）

方法：起一个隔离的 bench 实例（临时库副本 + 一次性 bench 凭据，不碰生产库、
不读 `.env`），用 Playwright/Chromium 驱动真实构建产物，在 keys 页（200 行窗口、
1319 条真实数据）用 `MutationObserver` 统计"勾一个复选框引起多少 DOM 变更"，
并测到下一帧的耗时。

| 指标 | 去掉 memo（旧形态） | 当前（memo 化） | 变化 |
|---|---|---|---|
| 一次勾选的 DOM mutation 数 | **603** | **6** | **-99%** |
| 被改动的节点数 | **200** | **1** | 整片 → 一行 |
| 勾选到下一帧 | 36.6 ms | 18.7–21.1 ms | **-42%** |
| 搜索输入一个字符到下一帧 | 31.9 ms | 31.5–32.3 ms | 基本不变 |

`changed_nodes 200 → 1` 是这组数据里最硬的一条：它直接证明"勾一个复选框
不再重渲染整张表"。

**搜索输入那一栏基本没变，这是诚实的结果而不是失败**：输入改变的是"哪些行可见"，
每行 props 都变了，memo 帮不上。要压它得换方案（虚拟列表或搜索防抖），
已列入待办，不混在这轮里。

### 13.4 顺带发现的一个构建期事实

`NEXT_PUBLIC_API_BASE_URL` 是**构建期烘进 bundle** 的常量（`lib/api.ts:5`）。
实测时把 bench 实例指到 :8010，页面仍然打向 :8000 —— 因为产物里写死了。
同源部署（Django 托管前端）不受影响，但"改个环境变量就能换后端地址"
对已构建的镜像不成立，需要重新 build。

### 13.5 现在的状态

§六 第 12 项已闭合，因此第 11 项（数据面视图改 async + 去掉 `race_chat` 的
`asyncio.run`）的前置条件已经满足 —— 它是 A1 那条结构性约束的唯一根治，
本轮仍未做（1–2 天量级，且会改变 `run_db` 的线程语义，值得单独一轮）。
